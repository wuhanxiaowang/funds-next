...
__name(createSupabase, "createSupabase");
var CORS_HEADERS = {
...
// api/news/schedule/status.js
var newsScheduleEnabled = false;
function getNewsScheduleEnabled() {
  return newsScheduleEnabled;
}
__name(getNewsScheduleEnabled, "getNewsScheduleEnabled");
function setNewsScheduleEnabled(v) {
  newsScheduleEnabled = !!v;
}
__name(setNewsScheduleEnabled, "setNewsScheduleEnabled");
...
// api/analyze/schedule/[path].js
var scheduleEnabled = false;
function onRequestOptions4() {
  return corsPreflight();
}
...
// api/analyze/stats.js
var emptyStats = /* @__PURE__ */ __name(() => ({ newsCount: 0, signalCount: 0, validSignalCount: 0, alertCount: 0 }), "emptyStats");
function onRequestOptions6() {
  return corsPreflight();
}
...
var defaultStatus = /* @__PURE__ */ __name(() => ({
  isRunning: false,
  currentStep: "",
  progress: 0,
  totalSteps: 5,
...
var analysisStatus = defaultStatus();
function onRequestOptions7() {
  return corsPreflight();
}
...
// api/asset-classes.js
function onRequestOptions9() {
  return corsPreflight();
}
...
// api/news.js
var DOMESTIC_RSS = ["https://feed.cnblogs.com/blog/sitehome/rss"];
      ...
      DOMESTIC_RSS.map(async (url) => {
        try {
          ...
          for (const entry of items) {
            ...
            const content = (entry.content || entry.title || "").slice(0, 1e4);
            const publishedAt = entry.pubDate ? new Date(entry.pubDate).toISOString() : (/* @__PURE__ */ new Date()).toISOString();
...
__name(fetchRssNews, "fetchRssNews");
function extractFeedTitle(xml) {
  const m = xml.match(/<title[^>]*>([^<]+)<\/title>/i);
  return m ? m[1].trim().slice(0, 200) : "";
}
__name(extractFeedTitle, "extractFeedTitle");
function parseRssItems(xml, limit) {
  const items = [];
  const itemRegex = /<item[^>]*>([\s\S]*?)<\/item>/gi;
  let match2;
  while ((match2 = itemRegex.exec(xml)) !== null && items.length < limit) {
    const block = match2[1];
    const title = block.match(/<title[^>]*>([\s\S]*?)<\/title>/i)?.[1]?.replace(/<[^>]+>/g, "").trim() || "";
    const desc = block.match(/<description[^>]*>([\s\S]*?)<\/description>/i)?.[1]?.replace(/<[^>]+>/g, "").trim() || block.match(/<content:encoded[^>]*>([\s\S]*?)<\/content:encoded>/i)?.[1]?.replace(/<[^>]+>/g, "").trim() || "";
    const pubDate = block.match(/<pubDate[^>]*>([^<]+)<\/pubDate>/i)?.[1] || block.match(/<dc:date[^>]*>([^<]+)<\/dc:date>/i)?.[1];
    items.push({ title, content: desc || title, pubDate });
  }
  return items;
}
__name(parseRssItems, "parseRssItems");
function onRequestOptions10() {
  return corsPreflight();
}
...
async function onRequestGet9(context) {
  ...
  try {
    ...
    const skip = parseInt(url.searchParams.get("skip") || "0", 10);
    const limit = Math.min(parseInt(url.searchParams.get("limit") || "100", 10), 500);
    const { data, error } = await supabase.from("signals").select("*").order("created_at", { ascending: false }).range(skip, skip + limit - 1);
    if (error) return errorResponse(error.message, 500);
    return jsonResponse(data || []);
  } catch (e) {
    return errorResponse(e.message || "Internal error", 500);
...
// ../.wrangler/tmp/pages-2MLaNT/functionsRoutes-0.5395718450197673.mjs
var routes = [
  {
...
// C:/Users/pc/AppData/Roaming/npm/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// C:/Users/pc/AppData/Roaming/npm/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
