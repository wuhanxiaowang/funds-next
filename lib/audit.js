...
async function initAuditTable() {
  ...
  try {
    ...
    const { error } = await supabase
      .rpc('create_audit_logs_table')
    
    if (error) {
      // 如果RPC函数不存在，尝试直接创建表
      // 注意：这在Edge Runtime中可能不工作，因为需要PostgreSQL权限
      console.warn('Failed to create audit logs table via RPC:', error)
      return false
    }
...
export async function getAuditLogs(options = {}) {
  if (!supabase) {
    return { logs: [], error: null, total: 0 }
  }
