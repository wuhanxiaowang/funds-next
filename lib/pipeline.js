...
import { logAudit, AUDIT_TYPES, AUDIT_OPERATIONS } from './audit'

// 更新分析状态 - 直接使用Supabase，避免API调用问题
async function updateAnalysisStatus(status) {
  try {
    if (!supabase) {
      console.warn('Supabase not configured, cannot update status')
      return
    }
    
    // 直接写入Supabase，避免通过API调用
    ...
    console.log('Status updated:', nextStatus.isRunning, nextStatus.currentStep)
  } catch (e) {
...
export async function runAnalysis(pageSize = 10) {
  ...
  for (let i = 0; i < newsListWithId.length; i++) {
    ...
    // 记录系统操作审计日志 - 生成信号
    await logAudit(AUDIT_TYPES.SYSTEM_ACTION, AUDIT_OPERATIONS.GENERATE_SIGNAL, {
      news_id: news.id,
      signal_id: sigRow.id,
      event: eventInfo.event,
      asset_class: eventInfo.asset_class,
